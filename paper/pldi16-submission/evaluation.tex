...
course of the first three assignments, from which we extracted 4,407
...
\caption{Results of our coverage testing and the distribution of test
  outcomes. Our random search successfully finds witnesses for over 85\%
  of the programs at the lowest timeout and improves slightly to 88\% at
  the next level. In both datasets we detect actual type errors about
  82\% of the time, unbound variables or constructors 3-4\% of the time,
  and diverging loops 2-3\% of the time. For the remaining 11-12\% of
  the programs we are unable to provide any useful feedback.  }
...
%
In both datasets our tool was able to find a witness for at least 87\%
of the programs in under 10 seconds.
%
...
\item The number forward (or backward) jumps that can be taken along the
  path from the witness to the stuck term. This is not quite the same as
  a stack-trace as it shows \emph{all} function calls, including those
  that returned successfully.
  % These abstract many details of the computation that are often
  % uninteresting or irrelevant to the explanation.
% \item \ES{others?}
\end{enumerate}
%
\begin{figure}[t]
...
Our algorithm does check for infinite types inside \forcesym, but there
are some degenerate cases where it is unable to detect them. Consider,
for example, the following buggy version of @replicate@ which is
supposed to produce a list containing @n@ copies of @x@.
%
\begin{code}
...
\nanomaly happily recurses as many times as necessary. Strictly
speaking, this function itself cannot ``go wrong'', the program would
not get stuck until a \emph{client} attempted to use the result
expecting a flat list. But this is not very satisfying as @replicate@ is
clearly to blame. Furthermore, in our experience, infinite-type errors
are some of the more difficult ones to debug (and to explain to
novices), so better support for this scenario would be useful.
...
would likely help alleviate this issue. This would allow users to
