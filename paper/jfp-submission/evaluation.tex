...
\item \emphbf{Witness-based Blame}
...
\caption{Results of our coverage testing and the distribution of test
  outcomes. Our random search successfully finds witnesses for 75--82\% of
  the programs in under one second, improving to 84\% in under
  10 seconds. In both datasets we detect actual type errors at least 75\%
  of the time, unbound variables or constructors 4--5\% of the time, and
  diverging loops 3\% of the time. For the remaining 13--16\% of the
...
2,745 (\resp 986) and a median of 17 (\resp 17).
%
The average number of jumps per trace is 7 (\resp 7) with a
maximium of 353 (\resp 185) and a median of 4 (\resp 4).
%
...
\begin{ecode}
  ...
  let rec digitsOfInt n =
    if n <= 0 then
      ...
      append ((*@\hlSherrloc{digitsOfInt (n / 10)}@*))
             [(*@\hlOcaml{n mod 10}@*)]
\end{ecode}
%
%\pagebreak
