...
$\evalfn{e}$ returns the \emph{list}
...
defined in Figure~\ref{fig:algo-gen-args},
that yields a list of holes $v_1, \ldots, v_n$ such that
$\eapp{e}{v_1 \ldots v_n}$ \emph{does not} evaluate to a lambda.
%
...
\quad \mbox{\textbf{where}} &    & \\
  \quad \quad \loopfn{vs} & = & \mbox{\textbf{case }} \evalfn{\eapp{e}{vs}} \mbox{\textbf{ of}} \\
             &   & \quad \triple{\efun{x}{e}}{\vsu}{\tsu},\ldots \rightarrow \loopfn{\vhole{\thole} :: vs} \\
             &   & \qquad (\ehole, \thole \mbox{ are fresh}) \\
             &   & \quad \_ \rightarrow vs
\end{array}
$$
...
%              _          -> vs
% \end{mcode}
\caption{Finding the number of arguments for the target expression.}
...
\quad \quad res    & = & \takefn{n}{\evalfn{\eapp{e}{vs}}} & (2) \\
