...
the resulting tree is well-typed. For a polymorphic type
$\ttree{\thole}$, eg in \recasegoodone, we will
place holes in the generated nodes; they will be lazily
...
\end{lem}
\begin{proof}
  We can construct $v$ from $\trace$ by induction on $k$, the length of $\trace$.
  %
  The goal is to find the step at which the refinement of $\vhole{\thole}$,
  $\resolve{\vhole{\thole}}{\vsu}$ becomes incompatible with $t$.
  %
  The base case, $k = 0$ is trivial, $\vsu_0 = \emptysu$ and
  $\resolve{\vhole{\thole}}{\emptysu} = \vhole{\thole}$ which is compatible with
  \emph{any} type $t$.
  %
  In the inductive case, let
  $$\trace_{k-1} = \triple{\eapp{f}{\vhole{\thole}}}{\emptysu}{\emptysu},\ldots,\triple{e_{k-1}}{\vsu_{k-1}}{\tsu_{k-1}}$$
  be the prefix of $\trace$ of length $k-1$.
  %
  Let us split cases on whether $\resolve{\vhole{\thole}}{\vsu_{k-1}}$ is compatible with $t$.
  %
  \begin{description}
  \item[Case $\vincompat{\resolve{\vhole{\thole}}{\vsu_{k-1}}}{t}$:]
    %
    The inductive hypothesis applies.
    %
  \item[Case $\vcompat{\resolve{\vhole{\thole}}{\vsu_{k-1}}}{t}$
        but  $\vincompat{\resolve{\vhole{\thole}}{\vsu_{k}}}{t}$:]
    %
    In this case we will show that $\resolve{\vhole{\thole}}{\vsu_{k-1}}$
    must contain some other hole $\ehole'$ which is instantiated with a
    concrete value at step $k$.
    %
    Furthermore, $\ehole'$ is instantiated in such a way that
    $\vincompat{\resolve{\vhole{\thole}}{\vsu_{k}}}{t}$.
    %
    Finally, we will show that if we had instantiated $\ehole'$ with a
    value such that
    $\vcompat{\resolve{\vhole{\thole}}{\vsu_{k}}}{t}$, the current
    ...
    $\vsu_{k} = \vsu_{k-1}[\ehole_1 \mapsto v_1] \ldots [\ehole_n \mapsto v_n]$.
    %
    We will assume, without loss of generality, that
    $\vsu_{k} = \vsu_{k-1}[\ehole' \mapsto v']$.
    ...
    $$
    %
    Since $\vsu_{k-1}$ and $\vsu_{k}$ differ only in $\ehole'$ but the resolved
    values differ,
    $\resolve{\vhole{\thole}}{\vsu_{k-1}} = \vcompat{\invctx{\ehole'[\thole']}}{t}$
    and
    $\resolve{\vhole{\thole}}{\vsu_{k}} = \vincompat{\invctx{v'}}{t}$.
    %
