...
\ES{explain the subtlety of narrowing a value hole to an uninstantiated type}
%
\begin{figure*}[ht]
$$
\begin{array}{lcl}
...
the first case of \forcesym\ we do not need to \forcesym\ the result of the
...
(denoted by $\vhole{\thole}$). \ES{note that $\gen{\ttree{t}}{\tsu}$ will
always place holes in the generated nodes, they will be filled in later,
on demand.}
...
\paragraph{The Type of a a Value} The (dynamic) type of a
...
We can think of the evaluation of \eapp{f}{\vhole{\thole}} as (dynamically)
...
% one of them can be obtained by
% suitably filling the holes of the other (\ie by suitably ``instantiating''
% the type variables appearing in the other).
%
...
\item [Case \tcompat{s}{t}:]
  ...
  $\eapp{f}{v}$ gets stuck. \ES{here we also need a repeated application of the substitution, as \ehole may map directly to $\vnode{\thole}{\vhole{\thole}_1}{\vleaf{\thole}}{\vleaf{\thole}}$ and $\ehole_1$ may later map to \etrue.}
