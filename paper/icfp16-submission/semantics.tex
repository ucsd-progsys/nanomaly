...
\begin{proof}
  ...
  types differ, we have, for some type context $T$,
  $\ptype{\trace_{i-1}}{f} = \intctx{\thole'}$
  and
  $\ptype{\trace_i}{f} = \intctx{t'}$.
  %
  We prove, in Lemma~\ref{lem:context-compat},
  that \RJ{rephrase lamma here}.
  Thus, by Lemma~\ref{lem:context-compat}, let $s$ be a
  concrete type such that $\intctx{s} = t$.
  %
