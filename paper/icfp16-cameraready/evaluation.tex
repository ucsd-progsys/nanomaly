...
%
We presented the students with ill-typed \ocaml\ programs and
asked them to
%
...
study not every student completed every problem. The number of
incomplete submissions ranges from 2 (for the \ocaml version of
@sumList@) to 17 (for the \toolname version of @wwhile@). Collecting
more responses per test pair was not possible, as it would require
having students answer the same problem twice (once with \ocaml and once
with \toolname).
