\subsubsection{Qualitative}\label{sec:advantage-traces}
...
and (4)~the jump-compressed trace produced by \toolname (in Figure~\ref{fig:traces}).
...
\end{verbatim}
%
\toolname\ produces a trace (see Figure~\ref{fig:traces}) showing how the evaluation of
@sqsum [1]@ gets stuck.
%
% \begin{center}
%   \includegraphics[height=125px]{sqsum.png}
...
how @sumList [1; 2]@ gets stuck at @2 + []@ (see Figure~\ref{fig:traces}).
%
% \begin{center}
%   \includegraphics[height=125px]{sumlist.png}
% \end{center}
...
a trace (see Figure~\ref{fig:traces}) that illustrates the true error on line 4, by
...
\end{verbatim}
%
\toolname\ synthesizes a trace (see Figure~\ref{fig:traces}) that draws the eye to the
